...
int selected_scenario = 0;  // Define global variable
...
void Board_self_test(void) {
  ...
  int scenario = scenario_select(-1);  // Use global default
  LCD_Clear(BLACK);
  LCD_ShowString(30, 20, (u8*)"Selected Scenario:", WHITE);
  char buf[10];
  sprintf(buf, "%d", scenario+1);
  LCD_ShowString(70, 20, (u8*)buf, GREEN);
  delay_1ms(2000);
